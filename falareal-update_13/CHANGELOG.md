...
## v11.29.0 — Seção "e-book grátis" pra gerar mais leads

**O quê:** Nova seção na landing page oferecendo o e-book gratuito
"Domine Idiomas: Aprenda com Wagnerakr" em troca do e-mail da pessoa —
mais um jeito de captar lead além da fila de espera. A introdução do
e-book também foi reescrita (gancho mais forte logo na abertura + uma
ponte pra FalaReal no fim do texto, pra quem quer alemão especificamente).

**Onde:** nova `<section id="ebook-gratis">`, logo antes da faixa final
de e-mail (`#comecar`), visível em qualquer idioma de aprendizado
(alemão ou inglês — não é dividida por `lang-de`/`lang-en` porque o
e-book cobre idiomas em geral). O PDF em si fica em
`/ebook/domine-idiomas-aprenda-com-wagnerakr.pdf`.

**Como funciona:** o formulário reaproveita o mesmo endpoint do
Formspree que já é usado na waitlist (`FORMSPREE_ENDPOINT`), mandando
um campo `origem: "ebook FalaReal"` pra diferenciar esse lead dos
outros nos e-mails que chegam. Ao enviar com sucesso, o formulário some
e aparece um botão de download do PDF (sem gate real no arquivo — ele é
público — a barreira do e-mail é só pra capturar o lead antes de
liberar o link). Textos da seção entraram em `I18N_STRINGS` (chaves
`ebook*`) nos três idiomas de interface (pt/en/es), seguindo a mesma
receita já documentada pra internacionalização.
